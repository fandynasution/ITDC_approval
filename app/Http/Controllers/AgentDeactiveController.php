...
use App\Mail\AgentDeactiveMail;
...
class AgentDeactiveController extends Controller
{
        ...
        try {
            ...
            if(isset($emailAddresses) && !empty($emailAddresses) && filter_var($emailAddresses, FILTER_VALIDATE_EMAIL)) {
                ...
                foreach ($emails as $email) {
                    Mail::to($email)->send(new AgentDeactiveMail($dataArray));
                }
    ...
    public function changestatus($status='', $entity_cd='', $doc_no='', $level_no='', $payment_code='')
    {
        ...
        return view('emails/agentdeactive/action', $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        if($status == 'A') {
            ...
            $sth->execute();
            if ($sth == true) {
                $msg = "You Have Successfully Cancelled the Approval Sales Agent Deactive No. ".$doc_no;
                $notif = 'Cancelled !';
                $st = 'OK';
                $image = "reject.png";
            } else {

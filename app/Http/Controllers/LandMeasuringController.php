...
class LandMeasuringController extends Controller
{
    ...
    public function changestatus($status='', $entity_cd='', $doc_no='', $level_no='')
    {
        ...
        return view('emails/landmeasuring/action', $data);
    }
    ...
    public function update(Request $request)
    {
        ...
        if($status == 'A') {
            ...
            $sth = $pdo->prepare("SET NOCOUNT ON; EXEC mgr.xrl_send_mail_approval_land_measuring ?, ?, ?, ?, ?;");
            ...
            $sth->execute();
            if ($sth == true) {
                $msg = "You Have Successfully Cancelled the Measuring Doc. No. ".$doc_no;
                $notif = 'Cancelled !';
                $st = 'OK';
                $image = "reject.png";
            } else {
